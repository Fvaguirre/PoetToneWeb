...
if __name__ == "__main__":
    ...
    for li in line:
        ...
        sp=li.split()
        if (not("".join(sp[-1:]) is None or  len("".join(sp[-1:])) == 0 or "".join(sp[-1:])==" ")):
            Set.append(("".join(sp[-1:])).lower())        
    
    for word in Set:
        if(i%4==0):
            if(not(i==0)):
                four_set.append(temp)
            temp = []
        temp.append(word)
        i+=1
    four_set.append(temp)
    ...
    lock = Lock()
    tempo = []
    for each in four_set:
        Process(target=rhymetype, args=(lock, each)).start()
